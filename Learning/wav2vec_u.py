...
# -----------------------------------------------------------------------------
# 1. THE SEGMENTER
# Purpose: Acoustic features repeat quickly. The Segmenter blocks collapse 
# consecutive identical repeating cluster predictions to build precise phonemes
# over time without penalty to model alignment.
# -----------------------------------------------------------------------------
...
class UniformRandomSegmenter(Segmenter):
    def pre_segment(self, dense_x, dense_padding_mask):
        ...
        if self.cfg.mean_pool:
            ...
            dense_padding_mask = dense_padding_mask.gather(
                2, index=indices
            ).reshape(bsz, -1)
        return dense_x, dense_padding_mask
...
# -----------------------------------------------------------------------------
# 2. THE GENERATOR
# Purpose: Predicts phoneme representations from dense acoustic inputs.
# Uses 1D Convolutions combined with Softmax mapping over the vocab size.
# -----------------------------------------------------------------------------
class Generator(nn.Module):
    ...
    def set_temp(self, num_updates):
        self.curr_temp = max(
            self.max_temp * self.temp_decay ** num_updates, self.min_temp
        )

    def _bn_padded_data(self, feature, padding_mask):
    ...
    def _normalize(self, dense_x):
        ...
        avg_probs = torch.softmax(
            dense_x.reshape(-1, csz).float(), dim=-1
        ).mean(dim=0)
    ...
    def forward(self, dense_x, dense_padding_mask, segment=True, dense_x_only=False):
        result = {}

        if segment:
            dense_x, dense_padding_mask = self.segmenter.pre_segment(
                dense_x, dense_padding_mask
            )

        if self.batch_norm:
            dense_x = self._bn_padded_data(dense_x, dense_padding_mask)
        if self.residual:
        ...
        result["dense_logits"] = dense_x
        code_perplexity = None
        prob_perplexity = None

        if not (self.no_softmax and dense_x_only):
            dense_x, code_perplexity, prob_perplexity = self._normalize(dense_x)

        result["dense_x"] = dense_x
        result["dense_padding_mask"] = dense_padding_mask
        result["code_perplexity"] = code_perplexity
        result["prob_perplexity"] = prob_perplexity

        return result


# -----------------------------------------------------------------------------
# 3. REAL DATA / TEXT (G2P)
# Purpose: Loads actual written text arrays mapped via G2P.
# The Discriminator scores the Generator against this baseline truth.
# -----------------------------------------------------------------------------
class RealData(nn.Module):
    def __init__(self, output_dim, pad_idx):
        super().__init__()
        self.output_dim = output_dim
        self.pad_idx = pad_idx

    def forward(self, tokens):
        token_padding_mask = tokens == self.pad_idx
...
# -----------------------------------------------------------------------------
# 4. THE DISCRIMINATOR
# Purpose: Uses nested Convolution blocks to inspect sequence likelihoods.
# Attempts to catch the Generator making linguistic errors or failing to speak
...
# -----------------------------------------------------------------------------
# 5. WAV2VEC-U (CORE GAN ARCHITECTURE)
# Purpose: The orchestrator tying Generator, Discriminator, and Real text.
# Calculates Gradient Penalty, Code Perplexity, and Smoothness to prevent
...
class Wav2vec_U(BaseFairseqModel):
    def __init__(self, cfg: Wav2vec_UConfig, target_dict):
        ...
        self.generator = Generator(cfg.input_dim, output_size, cfg)
        ...
        if self.mmi_weight > 0:
            ...
            self.decoder = nn.Linear(cfg.input_dim, cfg.target_dim)
    ...
    def set_num_updates(self, num_updates):
        ...
        self.generator.set_temp(num_updates)
    ...
    # Computes forward GAN loss passes and applies adversarial penalties
    def forward(
        self,
        features,
        padding_mask,
        ...
        gen_result = self.generator(
            features, padding_mask, segment=segment, dense_x_only=dense_x_only
        )

        dense_x = gen_result["dense_x"]
        dense_padding_mask = gen_result["dense_padding_mask"]
        dense_logits = gen_result["dense_logits"]
        code_perplexity = gen_result["code_perplexity"]
        prob_perplexity = gen_result["prob_perplexity"]
